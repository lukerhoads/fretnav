...
const FretboardPane = ({ patternName = "", tuning = ["E", "A", "D", "G", "B", "E"] }: Props) => {
    ...
    const [selectedNoteString, setSelectedNoteString] = useState<number | null>(null)
    ...
    return (
        <div className={styles.container}>
            {pattern && <div className={styles.pattern_info}>
                ...
                <a className={styles.explore} href={`/app?tab=navigator&pattern=${hyphenate(pattern.name)}`}><button>Explore</button></a>
            </div>}
            ...
            <Fretboard
                ...
                overlaidPatterns={[]}
                moveable={pattern ? pattern.moveable ? pattern.moveable : false : false}
                lefty={false}
                showNotes={showNotes}
                mutedStrings={pattern ? pattern.mutedStrings ? pattern.mutedStrings : [] : []} 
                resizeToHighlight={true}
                onPositionHighlight={onPosHighlight}
                />

            { selectedNoteFret && selectedNoteString && <div className={styles.note_info}>
